...
class WaterworldBase:
    ...
    def get_spaces(self):
        ...
        # Extend observation space to include communication signals from other agents
        extended_obs_dim = obs_dim + (self.n_pursuers - 1) # Additional dimensions for communication signals
        ...
        low=np.float32(-1.0),
        high=np.float32(1.0),
        shape=(3,),  # 2 for thrust, 1 for communication
        dtype=np.float32,
    )
    ...
    def step(self, action, agent_id, is_last):
        ...
        self.control_rewards = (
            (accel_penalty / self.n_pursuers) * np.ones(self.n_pursuers) * (1 - self.local_ratio)
        )
    ...
    def observe(self, agent_id):
        # Collect communication signals from other agents
        communication_signals = [p.emit_signal() for p in self.pursuers if p != self.pursuers[agent_id]]
        
        # Combine environmental observations with communication signals
        return np.concatenate([np.array(self.last_obs[agent_id], dtype=np.float32), communication_signals])
